...
class naive_bayes_classifier:
    """docstring for naive_bayes_categorical"""
    def __init__(self, split: float = .1, m: str = 'categorical') -> None:
        """
        initialies the class
        :param: split the ratio of data train to test
        """
        self.x_train = None
        self.y_train = None
        self.x_test = None
    ...
    def __str__(self) -> str:
        """
        returns the class as a string readable format
        :return: string of variables
        """
        if len(self.y_pred) > 1 and self.method != 'bernoulli':
            op = '''
Method: {},
...
Accuracy = {}
Kendall Tau = {}
Win rate = {}'''.format(self.method, self.data_split, self.y_pred, self.y_test, mse(
...
Mean squared error = {}
root mean squared error = {}
Win rate = {}'''.format(self.method, self.data_split, self.y_pred, self.y_test,
                        mse(self.y_test, self.y_pred),
                        self.RMSE(self.y_pred, self.y_test),
                        str(self.win_rate(self.y_test, self.y_pred) * 100) + '%')
        return op
    ...
    def split_data_classification(self, filename: str = 'NVDA_long.csv') -> None:
        ...
        print('\nlength of data', len(data), ', splits', len(
    ...
    def many_naive_bayes(self, num_iter: int = 10) -> None:
        """
        """
        win_rates = []
...
def main():
    ...
    # .001 gets one prediction
    gaussian = naive_bayes_classifier(.1, 'gaussian')
    gaussian.split_data_classification('NVDA_long.csv')
    gaussian.run_naive_bayes()
    # gaussian.many_naive_bayes(10)

    multinomial = naive_bayes_classifier(.1, 'multinomial')
    multinomial.split_data_classification('NVDA_long.csv')
    multinomial.run_naive_bayes()
    # multinomial.many_naive_bayes(10)

    complement = naive_bayes_classifier(.1, 'complement')
    complement.split_data_classification('NVDA_long.csv')
    complement.run_naive_bayes()
    # complement.many_naive_bayes(10)

    bernoulli = naive_bayes_classifier(.1, 'bernoulli')
    bernoulli.split_data_classification('NVDA_long.csv')
    bernoulli.run_naive_bayes()
    # bernoulli.many_naive_bayes(10)

if __name__ == '__main__':
    main()
else:
    print('Naive Bayes Imported')       
